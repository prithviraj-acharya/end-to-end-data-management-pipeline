...
def insert_data(conn):
    try:
        df = pd.read_csv("data/raw/local/customer_churn_dataset_local.csv", nrows=10000)
...
def connect_rds():
    try:
        print(f"Connecting to: {os.getenv('DB_HOST')}, {os.getenv('DB_USER')}, {os.getenv('DB_NAME')}") #added print statement.
        conn = pg8000.connect(
